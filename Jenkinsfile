		    ...
		    steps {
                ...
                // echo "${IP}"
              		...
              		sshagent(['SSH_AUTH']) {
		       		...
		       		sh("ssh -tt -o StrictHostKeyChecking=no ubuntu@${IP[0..-2]} sudo aws ecr get-login-password --region us-east-1 | sudo docker login --username AWS --password-stdin ${ECR}") 
                       		sh("ssh -tt -S -o StrictHostKeyChecking=no ubuntu@${IP[0..-2]} sudo docker run -d -p 8080:8080 --name container ${ECR}/project:app-V${BUILD_NUMBER}")
	       		} 
