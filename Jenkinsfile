		    ...
		    steps {
                ...
                // echo "${IP}"
              		...
              		sshagent(['SSH_AUTH']) {
		       		...
		       		//sh("ssh -tt -o StrictHostKeyChecking=no ubuntu@${IP[0..-2]} /bin/bash & sudo aws ecr get-login-password --region us-east-1 | sudo docker login --username AWS --password-stdin ${ECR}") 
