		    ...
		    steps {
                ...
                // echo "${IP}"
              		...
              		sshagent(['SSH_AUTH']) {
		       		...
		       		sh("ssh -tt -o StrictHostKeyChecking=no ubuntu@${IP[0..-2]} << EOF sudo aws ecr get-login-password --region us-east-1 | docker login --username AWS --password-stdin ${ECR}; docker pull ${ECR}/project:app-V${BUILD_NUMBER}; docker run -d -p 8080:8080 --name appcontainer ${ECR}/project:app-V${BUILD_NUMBER} EOF") 
